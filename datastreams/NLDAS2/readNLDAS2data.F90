...
!Computing a 1m root zone soil moisture value
       ...
       do index = 1,nvars
          ...
          if ((pid(index).eq.snowf_index).and.((LVT_MOC_SNOWF(source).ge.1).or.&
...
! Grab the NLDAS-2 VIC top-1 meter soil moisture directly. - Mocko
    ...
    if(LVT_MOC_TWS(source).ge.1) then 
       ...
       call LVT_logSingleDataStreamVar(LVT_MOC_TWS,source,varfield,vlevel=1,          &
            units="mm")
    endif
    ...
    do r = 1, nr
       do c = 1, nc
          if (ewind_f(c + (r-1)*nc) .gt. -9998 .and. &
               nwind_f(c + (r-1)*nc) .gt. -9998) then
             wind_f(c + (r-1)*nc) = sqrt(ewind_f(c + (r-1)*nc)**2 + &
                  nwind_f(c + (r-1)*nc)**2)*86400*0.001
          end if
       end do ! c
    end do ! r
    call interp_nldas2var(source,nc,nr, wind_f, varfield)
